...
const Home = () => {
    const apiKey = "b97316ed479ee4226afefc88d1792909";
    ...
    useEffect(() => {
        ...
        fetch(`https://api.themoviedb.org/3/genre/movie/list?api_Key=${apiKey}`).then(
            (Response) =>
    ...
    useEffect(() => {
        ...
        if (selectedGenres.length > 0) {
            fetch(
                `https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_Key=${apiKey}&release_date.lte=2019-12-12&with_genres=${encodeURI(
                    selectedGenres.join(",")
                )}`
            ).then((Response) =>
